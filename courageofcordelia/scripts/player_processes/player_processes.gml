...
function check_fire(){
	if fire {
		...
		if can_attack && !obj_inventory.can_show_inv && !o_trader_zone.in_trader_zone{
			...
			//shell casings
			if my_weapon = global.WeaponList.sidewinder or my_weapon = global.WeaponList.vector or my_weapon = global.WeaponList.fn{
...
function metal_door_collision(){
	...
	//move as far as we can in x and y before hitting the solid
	repeat(_disx){
		if !place_meeting(x + sign(_tx - x), y, o_metal_door) x += sign(_tx - x);
	}
		
	repeat(_disy){
		if !place_meeting(x, y + sign(_ty - y), o_metal_door) y += sign(_ty - y);
	}

...
if (can_show_inv) {
    ...
    if (mouse_check_button_released(mb_left)) {
        ...
        for (var i = 0; i < chest_rows * chest_collumns; i++) {
            var column = i % chest_collumns;
            var row = i div chest_collumns;
            
            var _posX = chest_slot_posX + (column * chest_sep);
            var _posY = chest_slot_posY + (row * chest_sep);
            ...
            if (is_between(mx, (CameraX() + _posX), (CameraX() + _posX) + 24)) {
                if (is_between(my, (CameraY() + _posY), (CameraY() + _posY) + 24)) {
                    ...
                    if (chest_index < array_length(chest_item)) {
                        show_debug_message("unequipping " + chest_item[chest_index].name);
                        unequip_item(chest_item[chest_index]);
                        break;

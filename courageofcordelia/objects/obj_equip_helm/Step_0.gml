...
if (can_show_inv) {
    ...
    if (mouse_check_button_released(mb_left)) {
        ...
        for (var i = 0; i < helm_rows * helm_collumns; i++) {
            var column = i % helm_collumns;
            var row = i div helm_collumns;
            
            var _posX = helm_slot_posX + (column * helm_sep);
            var _posY = helm_slot_posY + (row * helm_sep);
            ...
            if (is_between(mx, (CameraX() + _posX), (CameraX() + _posX) + 24)) {
                if (is_between(my, (CameraY() + _posY), (CameraY() + _posY) + 24)) {
                    ...
                    if (helm_index < array_length(helm_item)) {
                        show_debug_message("unequipping " + helm_item[helm_index].name);
                        unequip_item(helm_item[helm_index]);
                        break;

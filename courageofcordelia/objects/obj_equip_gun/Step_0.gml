...
if (can_show_inv) {
    ...
    if (mouse_check_button_released(mb_left)) {
        ...
        for (var i = 0; i < gun_rows * gun_collumns; i++) {
            var column = i % gun_collumns;
            var row = i div gun_collumns;
            
            var _posX = gun_slot_posX + (column * gun_sep);
            var _posY = gun_slot_posY + (row * gun_sep);
            ...
            if (is_between(mx, (CameraX() + _posX), (CameraX() + _posX) + 24)) {
                if (is_between(my, (CameraY() + _posY), (CameraY() + _posY) + 24)) {
                    ...
                    if (gun_index < array_length(gun_item)) {
                        show_debug_message("unequipping " + gun_item[gun_index].name);
                        unequip_item(gun_item[gun_index]);
                        break;
